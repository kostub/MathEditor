...
typedef enum {
    kMTSubIndexTypeNone  = 0,       // The index denotes the whole atom, subIndex is nil.
    kMTSubIndexTypeNucleus,         // The position in the subindex is an index into the nucleus
    kMTSubIndexTypeSuperscript,     // The subindex indexes into the superscript.
    kMTSubIndexTypeSubscript,       // The subindex indexes into the subscript
    kMTSubIndexTypeNumerator,       // The subindex indexes into the numerator (only valid for fractions)
    kMTSubIndexTypeDenominator,     // The subindex indexes into the denominator (only valid for fractions)
    kMTSubIndexTypeRadicand,        // The subindex indexes into the radicand (only valid for radicals)
    kMTSubIndexTypeDegree           // The subindex indexes into the degree (only valid for radicals)
...
// Create a range at level 0 from the give range.
+ (MTMathListRange *)makeRangeForRange:(NSRange)range;
// Make a range of length 1
+ (MTMathListRange*) makeRange:(MTMathListIndex*) start;
// Make a range of length 1 at the level 0 index start
...
// Union all ranges in the given array of ranges
